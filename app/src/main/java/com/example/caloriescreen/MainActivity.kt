...
fun CalorieScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Text(text = "Result: $result", color = MaterialTheme.colorScheme.secondary, fontWeight = FontWeight.Bold)

        Calculation(male = male, weight = weightInput.toIntOrNull() ?: 0, intensity = intensity) { newResult ->
            result = newResult
        }
    }
}
